...
bool Dynamixel::OpenPort() {
  if (port_handler_->openPort()) {
    ROS_INFO("Succeeded to open the port : %s!", port_name_);
  } else {
    ROS_INFO("Failed to open the port : %s!", port_name_);
    return false;
  }

  if (port_handler_->setBaudRate(baudrate_)) {
    ROS_INFO("Succeeded to change the baudrate : %s!", port_name_);
    return true;
  } else {
    ROS_INFO("Failed to change the baudrate : %s!", port_name_);
    return false;
  }
}
...
bool Dynamixel::ClosePort() {
  ...
  ROS_INFO("Close port : %s!", port_name_);
}
  ...
  if (status_return_level_ == 2) {
    ...
    while(port_handler_->getBytesAvailable() < 11) {
      if (port_handler_->isPacketTimeout()) {
        ROS_INFO("Read Error(time out): ID %d, available bytes %d", servo_id, port_handler_->getBytesAvailable());
...
int64_t Dynamixel::Read(uint8_t servo_id, DynamixelParameter dp) {
  ...
  if (status_return_level_ != 0) {
    ...
    if (read_length != 11+dp.size()) {
      ROS_INFO("Read Error(no data): ID %d, data_length = %d", servo_id, read_length);
      error_last_read_ = true;
      return 0;
    }
    if (read_data[0] != HEADER[0] or
      ...
      ROS_INFO("Read Error(header): ID %d", servo_id);
      error_last_read_ = true;
      return 0;
    }
    ...
    if (sum_est != sum_read) {
      ROS_INFO("Read Error(crc): ID %d, est:%d, read:%d", servo_id, sum_est, sum_read);
      error_last_read_ = true;
      return 0;
    }
...
uint8_t Dynamixel::SyncRead(uint8_t num_servo, uint8_t servo_id_list[], DynamixelParameter dp,
  ...
  for(int i_servo=0; i_servo<num_servo; i_servo++) {
    ...
    while(port_handler_->getBytesAvailable() < 11+dp.size()) {
      if (port_handler_->isPacketTimeout()) {
        ROS_INFO("Read Error(time out): ID %d, available bytes %d", servo_id_list[i_servo], port_handler_->getBytesAvailable());
        error_last_read_ = true;
        return 0;
      }
    }
    ...
    if (port_handler_->readPort(read_data, 11+dp.size()) == -1) {
      ROS_INFO("Read Error : ID %d", servo_id_list[i_servo]);
      continue;
    }
    if (read_data[0] != HEADER[0] or
        read_data[1] != HEADER[1] or
      ...
      ROS_INFO("Read Error(header): ID %d", servo_id_list[i_servo]);
      continue;
    }
    ...
    if (read_data[9+dp.size()] != sum_l or
      ...
      ROS_INFO("Read Error(crc): ID %d", servo_id_list[i_servo]);
      error_last_read_ = true;
      continue;
    }
    // 正常なデータ
